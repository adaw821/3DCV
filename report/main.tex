...
depth correlates at $r=0.94$ with the corresponding photo's depth.
...
\textbf{Data.} We collected nine building/architecture sketches from the web
(including the historical Field Lane engraving used by the base paper) and
seven real building photos. Following the base paper we also synthesize
sketches from photos with the ``Dodging''
...
recovers depth strongly correlated with the photo's depth ($r=0.94$),
...
\begin{table}[t]
  ...
  \begin{tabular}{lcc}
    ...
    \midrule
    Raw sketch (no enhancement)        & \textbf{0.939} & \textbf{0.099} \\
    + cleaning + CLAHE                 & 0.925 & 0.112 \\
    + synthetic shading                & 0.892 & 0.117 \\
    \bottomrule
  \end{tabular}
...
(Table~\ref{tab:degrade}). Degradation drops correlation from $0.94$ to
$0.87$; our cleaning$+$CLAHE stage not only undoes this damage but slightly
exceeds the clean-sketch baseline ($0.96$), because each operation is matched
to a specific corruption (illumination normalization $\rightarrow$ uneven
light, bilateral filter $\rightarrow$ noise, CLAHE $\rightarrow$ low
contrast). Adding synthetic shading again hurts. We conclude that \emph{cleaning is a real,
...
\begin{table}[t]
  ...
  \begin{tabular}{lcc}
    ...
    \midrule
    Clean sketch (upper bound)         & 0.939 & 0.099 \\
    Degraded, no enhancement           & 0.872 & 0.109 \\
    Degraded $+$ cleaning $+$ CLAHE    & \textbf{0.959} & \textbf{0.091} \\
    Degraded $+$ synthetic shading     & 0.893 & 0.117 \\
    \bottomrule
  \end{tabular}
...
\textbf{Field Lane case study.} To address the original motivating scenario,
we apply our method to two 19th-century engravings of Field Lane by different
artists (Fig.~\ref{fig:fieldlane}). Despite differing line styles, the dense
ECC criterion aligns them successfully and depth-space fusion yields a single
coherent street: the foreground figures and shopfronts are near, and the
alley recedes into the distance. This is exactly the case where the base
paper's feature-correspondence stitching fails, and it lets a viewer
``walk into'' a reconstructed early-1800s London street in our interactive
viewer.

\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{figures/fieldlane.png}
  \caption{Field Lane case study. Two engravings by different artists (top)
  are fused in depth space into one coherent street (bottom): a depth map and
  the reconstructed 3D surface, with the alley correctly receding into depth.}
  \label{fig:fieldlane}
\end{figure}

\begin{figure}[t]
  \centering
  \includegraphics[width=0.49\linewidth]{figures/recon_3d.png}\hfill
  \includegraphics[width=0.49\linewidth]{figures/viewer.png}
  \caption{Reconstruction of a building sketch. Left: the recovered depth
  surface (near $=$ high). Right: the sketch textured onto that surface,
  showing the facade lifting into relief. The live, interactive version runs
  in the browser (\texttt{web/}, Three.js): rotate, zoom, and adjust depth
  scale in real time.}
  \label{fig:viewer}
\end{figure}
...
($r=0.94$), with a targeted cleaning stage that measurably restores fidelity
...
\textbf{Wang Qiaopei}: FILL-IN -- e.g.\ sketch pre-processing
(\S\ref{sec:pre}), depth-model integration, evaluation and ablation.\\
\textbf{Peng Shiqi}: FILL-IN -- e.g.\ multi-sketch depth-space
fusion (\S\ref{sec:fuse}), interactive viewer (\S\ref{sec:viewer}), data
collection and figures.
